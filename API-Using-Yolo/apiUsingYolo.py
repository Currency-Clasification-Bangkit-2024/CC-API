...
# untuk tampilan html untuk input gambar
html_template = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    for i, box in enumerate(detections):
        ...
        if cls in nominal_mapping:
            ...
            temp_detections.append(
                {"nominal": nominal, "confidence": conf, "bbox": bbox}
            )

    unique_detections = []
    for det in temp_detections:
        bbox = det["bbox"]
        if not any(iou(bbox, u["bbox"]) > 0.5 for u in unique_detections):
...
def home():
    return render_template_string(
        html_template, detections=None, total_value=0, detection_info=None
    )
